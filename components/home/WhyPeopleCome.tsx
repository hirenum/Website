...
const WhyPeopleCome: React.FC<WhyPeopleComeProps> = memo(({ scrollToSection }) => {
  const painPointNodes: PainPointNode[] = useMemo(() => [
    { text: "They're doing great work, but very few people outside their circle know about it.", icon: User, top: "20%", left: "10%", align: "left", delay: 0.2 },
    { text: "They're relying too much on referrals and outbound, and want more inbound opportunities.", icon: FileText, top: "20%", right: "10%", align: "right", delay: 0.25 },
    { text: "Their competitors with less experience have more visibility and a louder voice in the market.", icon: Users, top: "50%", left: "2%", align: "left", delay: 0.3 },
    { text: "They know they should \"post on LinkedIn,\" but don't have the time, structure or strategy.", icon: Megaphone, top: "50%", right: "2%", align: "right", delay: 0.35 },
    { text: "They've tried posting randomly, but it hasn't turned into leads, clients, or real authority.", icon: Clock, top: "80%", left: "10%", align: "left", delay: 0.4 },
    { text: "They're ready to be seen as a go-to name in their industry, not just another professional.", icon: Target, top: "80%", right: "10%", align: "right", delay: 0.45, isHighlight: true }
  ], []);

  const mobilePainPointNodes: MobilePainPointNode[] = useMemo(() => [
  ...
  return (
    <section id="why-people-come" className="py-24 relative overflow-hidden theme-transition bg-[var(--bg-primary)]">
      {/* Simplified background - removed heavy blur */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="hidden lg:block absolute top-[45%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-gradient-to-r from-[#DC0078]/8 to-[#1BB8BD]/8 dark:from-[#DC0078]/5 dark:to-[#1BB8BD]/5 rounded-full blur-3xl"></div>
      </div>
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="hidden lg:block relative max-w-7xl mx-auto h-[700px] xl:h-[800px]">
          ...
          {/* SVG Tree Branches with animated balls */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            {[
              { path: "M 40 45 C 35 45, 35 20, 32 20", delay: 0.1, dur: "4s" },
              { path: "M 40 50 C 35 48, 30 52, 24 50", delay: 0.15, dur: "4.5s" },
              { path: "M 40 55 C 35 55, 35 80, 32 80", delay: 0.2, dur: "5s" },
              { path: "M 60 45 C 65 45, 65 20, 68 20", delay: 0.1, dur: "4.2s" },
              { path: "M 60 50 C 65 52, 70 48, 76 50", delay: 0.15, dur: "4.7s" },
              { path: "M 60 55 C 65 55, 65 80, 68 80", delay: 0.2, dur: "5.2s" },
            ].map((branch, idx) => (
              <g key={idx}>
                <motion.path
                  d={branch.path}
                  ...
                  viewport={{ once: true }}
                />
                <circle r="0.5" fill={idx < 3 ? "#DC0078" : "#1BB8BD"} opacity="0.8">
                  <animateMotion dur={branch.dur} repeatCount="indefinite" path={branch.path} />
                </circle>
              </g>
            ))}
          </svg>
          ...
          {painPointNodes.map((node, idx) => (
            <motion.div
              key={idx}
              className="absolute w-[220px] lg:w-[250px] xl:w-[300px] will-change-transform"
            ...
            >
              <div className="relative p-4 lg:p-5 xl:p-6 rounded-2xl border overflow-hidden group transition-colors duration-300 bg-transparent border-gray-300/50 dark:border-white/10 hover:bg-gradient-to-r hover:from-[#DC0078]/5 hover:to-[#1BB8BD]/5">
          ...
          <div className="relative pl-8 ml-4">
            ...
            {mobilePainPointNodes.map((node, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 p-4 rounded-xl border bg-transparent border-gray-300/50 dark:border-white/10">
